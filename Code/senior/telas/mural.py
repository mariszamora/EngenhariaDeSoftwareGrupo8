...
ARQUIVO_AVISOS = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "avisos.json")
    ...
    if "avisos" not in st.session_state:
        if os.path.exists(ARQUIVO_AVISOS):
            ...
            st.session_state.avisos = [
                {"id": 1, "titulo": "Bem-vindos!", "data": "01/06/2026", "conteudo": "A Comunidade 65+ está no ar!"},
                {"id": 2, "titulo": "Novo curso de Dança", "data": "15/06/2026", "conteudo": "Aulas de dança todas as terças e quintas, 10h."},
            ]
            _salvar_avisos()
...
def renderizar():
    ...
    st.markdown("<h1 style='color:#1a4263;'>📢 Mural de Avisos</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#4a5c6c;'>Fique por dentro das novidades da comunidade!</p>", unsafe_allow_html=True)
    st.divider()
        ...
        with col_acao:
            if eh_tutor:
                if st.button("🗑️", key=f"remover_{aviso['id']}", help="Remover aviso"):
                    st.session_state.avisos = [a for a in st.session_state.avisos if a['id'] != aviso['id']]
                    _salvar_avisos()
                    st.rerun()