...
def conteudo_modal(atividade_id):
    ...
    if eh_tutor:
        st.warning(
            "Deseja deletar esta atividade? Isso removerá as inscrições de todos os alunos."
        )
            ...
            if st.button("Sim, Deletar", use_container_width=True, key="btn_sim_del"):
                if deletar_atividade(atividade.id):
                    st.session_state.flash_msg = (
                        f"Atividade '{atividade.titulo}' deletada com sucesso!"
                    )
                else:
...
def renderizar():
    ...
    with col_center:
        st.markdown(
            f"<div style='text-align:center; font-size:1.4rem; font-weight:700; color:#1a4263; padding:10px 0;'>{
                nome_mes
            } {ano}</div>",
            unsafe_allow_html=True,
        )
            ...
            with cols[i]:
                if dia != 0:
                    with st.container(height=230, border=True):
                        st.markdown(
                            f"<div style='text-align:right; font-weight:700; color:#333; font-size:0.9rem; margin-bottom:4px;'>{
                                dia
                            }</div>",
                            unsafe_allow_html=True,
                        )
                        ...
                        if lista:
                            ...
                            if len(lista) > 1:
                                st.markdown(
                                    f"<div style='font-size:0.65rem; color:#888; text-align:center; padding-top:4px;'>+{
                                        len(lista) - 1
                                    } mais</div>",
                                    unsafe_allow_html=True,
                                )
                    ...
                    with st.container(height=230, border=True):
                        st.markdown(
                            "<div style='text-align:center; color:#ccc; margin-top:60px; font-size:1.5rem;'>-</div>",
                            unsafe_allow_html=True,
                        )
    ...
    if eh_tutor and st.session_state.mostra_painel_tutor:
        ...
        st.markdown('<div id="ancora-nova-atividade"></div>', unsafe_allow_html=True)
        import streamlit.components.v1 as components

        components.html(
            "<script>window.parent.document.getElementById('ancora-nova-atividade').scrollIntoView({behavior:'smooth'});</script>",
            height=0,
        )
        ...
        with st.form("form_criar_atividade", clear_on_submit=True):
            ...
            if salvar:
                if not all([titulo, data, horario, local]):
                    ...
                    st.session_state.mostra_painel_tutor = False
                    st.session_state.flash_msg = (
                        f"Atividade '{titulo}' criada com sucesso!"
                    )
                    st.rerun()
            ...
            with st.form("form_inscrever", clear_on_submit=True):
                ...
                escolha = st.selectbox(
                    ...
                    format_func=lambda a: (
                        f"{a.titulo} — {a.data} às {a.horario} ({a.local})"
                    ),
                )
                if st.form_submit_button(
                    "✅ Quero Participar", use_container_width=True
                ):
